...
const LandingPage = () => {
	return (
		<div className="min-h-screen bg-gradient-to-br from-slate-900 via-gray-900 to-slate-800 flex flex-col items-center justify-center px-4 py-12">
			<div className="max-w-3xl w-full text-center mb-16 animate-slideInUp">
				<div className="flex items-center justify-center mb-6">
					<div className="w-16 h-16 bg-gradient-to-br from-cyan-400 to-teal-500 rounded-full flex items-center justify-center shadow-2xl border-4 border-white/20">
						<span className="text-4xl">💰</span>
					</div>
				</div>
				<h1 className="text-5xl font-extrabold text-white mb-4 drop-shadow-lg">BudgetPlanner</h1>
				<p className="text-xl text-cyan-200 mb-6 font-medium">Your modern, secure, and beautiful personal finance dashboard</p>
				<Link to="/register" className="inline-block bg-gradient-to-r from-cyan-500 to-teal-600 text-white font-semibold py-3 px-8 rounded-xl hover:from-cyan-600 hover:to-teal-700 transform hover:scale-[1.03] transition-all duration-300 shadow-lg hover:shadow-xl">
					Get Started
				</Link>
			</div>
			...
			<div className="max-w-5xl w-full grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 mb-20 animate-slideInUp">
				{features.map((feature, idx) => (
					<div key={feature.title} className="glass rounded-2xl p-8 shadow-2xl border border-white/20 flex flex-col items-center text-center hover:bg-white/10 transition-all duration-300">
						<div className={`w-14 h-14 mb-4 bg-gradient-to-br from-cyan-400 to-teal-500 rounded-xl flex items-center justify-center shadow-lg`}>
							<span className="text-3xl">{feature.icon}</span>
						</div>
						<h3 className="text-2xl font-bold text-white mb-2">{feature.title}</h3>
						<p className="text-cyan-200 text-sm mb-2">{feature.description}</p>
					</div>
				))}
			</div>
			...
			<div className="max-w-2xl w-full text-center animate-slideInUp">
				...
				<ul className="text-cyan-200 text-md space-y-2 mb-8">
				...
				<Link to="/login" className="inline-block bg-gradient-to-r from-orange-500 to-amber-600 text-white font-semibold py-3 px-8 rounded-xl hover:from-orange-600 hover:to-amber-700 transform hover:scale-[1.03] transition-all duration-300 shadow-lg hover:shadow-xl">
